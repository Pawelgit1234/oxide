...
pub fn generate_routes(route_configs: &Vec<RouteConfig>) -> anyhow::Result<HashMap<String, Route>> {
    ...
    for route_config in route_configs.iter() {
        ...
        if let Some(index_path) = &route_config.index {
            ...
            let route = Route {
                route_type: RouteType::Response(response.status, response.body.clone()),
                gzip,
                timeout_ms,
            };
            routes.insert(route_config.path.clone(), route);
